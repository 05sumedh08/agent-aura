...
export default function AnalyticsPage() {
  ...
  async function loadAnalytics() {
    try {
      ...
      // Performance distribution (Derived from GPA)
      const perfDist: { [key: string]: number } = {};
      data.forEach(s => {
        let perfCategory = 'Below Average';
        if (s.gpa >= 3.5) perfCategory = 'Excellent';
        else if (s.gpa >= 3.0) perfCategory = 'Above Average';
        else if (s.gpa >= 2.5) perfCategory = 'Average';

        perfDist[perfCategory] = (perfDist[perfCategory] || 0) + 1;
      });
          ...
          <div className="space-y-3">
            {Object.entries(analytics.performanceDistribution).map(([performance, count]) => (
              <div key={performance} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-8 bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden">
                  <div
                    className={`h-full flex items-center justify-end px-3 transition-all duration-500 ${performance === 'Excellent' ? 'bg-gradient-to-r from-emerald-500 to-green-500' :
